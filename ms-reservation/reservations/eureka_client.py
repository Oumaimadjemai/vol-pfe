...
def wait_for_eureka(eureka_server, max_retries=5, delay=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"⚠️ Cannot connect to Eureka (attempt {attempt + 1}/{max_retries})")
...
def start_eureka_client():
    ...
    try:
        eureka_server = os.getenv("EUREKA_SERVER", "http://localhost:8888/eureka/").strip()
        if not eureka_server.endswith('/'):
            eureka_server += '/'

        app_name       = os.getenv("EUREKA_APP_NAME",      "MS-RESERVATION")
        instance_host  = get_instance_host()
        instance_port  = int(os.getenv("EUREKA_INSTANCE_PORT", "8001"))
