...
const kafkaProducer = require('./services/kafkaProducer');  // ⭐ ZID HAD
...
class App {
  constructor() {
    ...
    this.setupDatabase();
    this.setupKafka();     // ⭐ Kafka avant les routes
    this.setupRoutes();
    this.setupEureka();
  }
  ...
  // ⭐ NOUVELLE FONCTION: Connexion Kafka
  async setupKafka() {
    try {
      await kafkaProducer.connect();
      console.log('✅ Kafka Producer connecté avec succès');
    } catch (error) {
      console.error('⚠️ Kafka non disponible, continuer sans...', error.message);
    }
  }
